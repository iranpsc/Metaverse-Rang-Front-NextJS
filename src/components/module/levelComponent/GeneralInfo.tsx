  ...
  return (
    <>
      <div className="w-full sm:w-4/5 flex flex-wrap justify-between">
        ...
        />
        <DetailItem
          title={targetData(levelsTranslatePage, "required points")}
          value={generalInfo.data.score.toLocaleString()}
        />
        <DetailItem
          title={targetData(levelsTranslatePage, "surface model file size")}
          value={generalInfo.data.file_volume}
        />
        <DetailItem
          title={targetData(levelsTranslatePage, "level rank")}
          value={generalInfo.data.rank}
        />
        <DetailItem
          title={targetData(
            levelsTranslatePage,
            "the number of points used in the level model"
          )}
          value={generalInfo.data.points.toLocaleString()}
        />
        <DetailItem
          title={targetData(levelsTranslatePage, "number of sub-branches")}
          value={generalInfo.data.subcategories}
        />
        <DetailItem
          title={targetData(
            levelsTranslatePage,
            "number of surface model lines"
          )}
          value={generalInfo.data.lines.toLocaleString()}
        />
        <DetailItem
          title={targetData(levelsTranslatePage, "level creation date")}
        ...
        />
        <DetailItem
          title={targetData(levelsTranslatePage, "persian font used")}
          value={generalInfo.data.persian_font}
        />
        <DetailItem
          title={targetData(levelsTranslatePage, "surface designer")}
          value={generalInfo.data.designer}
        />
        <DetailItem
          title={targetData(levelsTranslatePage, "english font used")}
          value={generalInfo.data.english_font}
        />
        <DetailItem
          title={targetData(levelsTranslatePage, "3d model designer")}
          value={generalInfo.data.model_designer}
        />
        <DetailItem
          fullBox={true}
          title={targetData(levelsTranslatePage, "colors used")}
