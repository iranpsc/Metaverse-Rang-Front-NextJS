...
import { SectionTimerSkeleton } from "../../skeleton/SectionTimerSkeleton";
...
const SectionTimer = ({ params }: { params: Params }) => {
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        const activeTimedEvents = allEvents
          ...
          .sort((a, b) => jalaliToTimestamp(a.end!) - jalaliToTimestamp(b.end!));
        ...
        const endedTimedEvents = allEvents
          .filter((ev) => ev.end && jalaliToTimestamp(ev.end) <= now)
          .sort((a, b) => jalaliToTimestamp(b.end!) - jalaliToTimestamp(a.end!));

        const remainingCount = 4 - activeTimedEvents.length;

        const finalEvents = [...activeTimedEvents, ...endedTimedEvents.slice(0, remainingCount)];
    ...
    fetchEvents();
    // قبلاً این fetch مستقل از isInView اجرا می‌شد؛ الان به isInView وابسته‌ست
    // چون دیگه نیازی به فچ زودهنگام قبل از دیده‌شدن سکشن نیست.
  }, [isInView]);
  ...
  const isLoading = !isInView || events.length === 0;
  ...
  return (
    ...
    >
      ...
      {/* دکمه‌های موبایل — همیشه رندر می‌شن تا در حالت لودینگ جابجایی چیدمان (CLS) نداشته باشیم */}
      <div className="md:hidden flex items-center justify-between w-full absolute z-10 top-[91px] px-1">
        <div
          onClick={handlePrev}
          aria-disabled={isLoading}
          className={`bg-primary rounded-full p-[8px] cursor-pointer flex justify-center items-center transition-opacity ${
        ...
        <div
          onClick={handleNext}
          aria-disabled={isLoading}
          className={`bg-primary rounded-full p-[8px] cursor-pointer flex justify-center items-center ms-5 transition-opacity ${
            isLoading ? "opacity-50 pointer-events-none" : ""
          }`}
        >
      ...
      <div
        onClick={handlePrev}
        aria-disabled={isLoading}
        className={`bg-primary rounded-full p-[10px] cursor-pointer hidden lg:flex justify-center items-center transition-opacity ${
          isLoading ? "opacity-50 pointer-events-none" : ""
        }`}
      >
      ...
      {/* باکس ایونت: در حالت لودینگ اسکلت، در غیر این‌صورت محتوای واقعی */}
      {isLoading || !eventData ? (
        <SectionTimerSkeleton />
      ) : (
        <div
          style={{ background: "linear-gradient(-135deg, transparent, #9898a0 , transparent)" }}
          className="p-[1px] rounded-[48px] lg:rounded-[50px]"
        >
              ...
              <div className="h-full w-full py-5 md:py-0 lg:rounded-[48px] flex flex-col lg:flex-row gap-5 justify-center items-center">
                <div className="w-full h-full flex justify-center max-w-[380px] max-h-[100%] lg:rounded-xl lg:rounded-[32px] overflow-hidden relative">
                  <Image
                    ...
                    quality={75}
                  />
                  <div className="w-full h-full bg-gradient-to-b to-[#DEDEE9] via-[#dedee942] from-[#dedee905] md:bg-gradient-to-l md:to-[#DEDEE9] md:via-[#dedee942] md:from-transparent dark:bg-gradient-to-b dark:to-[#313131] dark:via-[#3131315b] dark:from-transparent md:dark:bg-gradient-to-l md:dark:to-[#313131] md:dark:via-[#3131315b] md:dark:from-[#31313100] absolute right-0"></div>
                </div>
                <div className="w-full lg:mt-0 text-justify flex flex-col justify-between h-full md:py-4">
                  ...
                  <p
                    className="w-fit text-start text-sm lg:text-base xl:text-xl 3xl:text-2xl text-matn-2 font-azarMehr font-medium ms-1 md:ms-5 line-clamp-4 mt-3 lg:mt-0"
                ...
                <div className="w-full flex flex-col">
                  {eventData.btnName && (
                    <a
                      ...
                      aria-label="meta btn"
                      href={eventData.link}
                      className="!w-full rounded-[28px] py-3 text-center lg:text-lg 3xl:text-[19px] text-white dark:text-black bg-primary font-azarMehr font-medium mt-5"
                    >
                      {eventData.btnName}
                    </a>
                  )}
      ...
      <div
        onClick={handleNext}
        aria-disabled={isLoading}
        className={`bg-primary rounded-full p-[10px] cursor-pointer hidden lg:flex justify-center items-center ms-5 transition-opacity ${
          isLoading ? "opacity-50 pointer-events-none" : ""
        }`}
      >
