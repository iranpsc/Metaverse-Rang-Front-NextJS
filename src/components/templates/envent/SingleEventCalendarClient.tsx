...
export default function EventCalendarClient({
   ...
   * Skeleton delay
   *
   * فقط برای اینکه Skeleton حتماً قابل مشاهده باشد.
   * UI اصلی کامپوننت تغییری نمی‌کند.
   */
  useEffect(() => {
  ...
  const grouped = monthEventsDay.reduce(
    (acc, event) => {
  ...
  // Fetch calendar events by month
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchCalendarEvents = async () => {
      try {
        ...
        // Redirect to calendar page with date parameter
        router.push(
          `/${params.lang}/calendar?date=${encodeURIComponent(
            selectedEventDate
          )}`
        );
      } catch (error) {
  ...
  function handleSearchClick() {
    ...
    fetch(url, { headers })
      ...
      .then((data) => {
        ...
        // Redirect to calendar page with search parameter
          ...
          >
            ...
            >
              <div className="flex items-center h-7 xl:h-9 2xl:h-10">
                <div className="whitespace-nowrap inline-block bg-transparent relative rounded-lg aspect-square h-full overflow-hidden">
                  <div className="bg-[#ff0000] absolute top-[72%] left-0 w-full h-[30%] rounded-t-md z-50" />
                  <div className="bg-[#0000ff] absolute top-[58%] left-0 w-full h-[40%] rounded-t-md z-40" />
                  <div className="bg-[#ffff00] absolute top-[38%] left-0 w-full h-[60%] rounded-t-md z-30" />
                  <div className="bg-[#80ff00] absolute top-[18%] left-0 w-full h-[80%] rounded-t-md z-20" />
                  <div className="bg-[#ff00ff] absolute top-0 left-0 w-full h-full rounded-t-md z-10" />
                </div>
            ...
            {bluePercent !== 0 && (
              <div className="flex items-center justify-between h-11 2xl:h-12 px-2 w-[96%] rounded-md item sm:h-[34px] sm:text-sm lg:text-base xl:text-lg 2xl:text-xl xl:h-11">
                <div className="flex items-center h-full">
                  <div className="bg-[#0000ff] rounded-lg aspect-square h-7 xl:h-9 2xl:h-10" />
                  <span className="mx-2 whitespace-nowrap">
            ...
            {redPercent !== 0 && (
              <div className="flex items-center justify-between h-11 2xl:h-12 w-[96%] px-2 rounded-md item sm:h-[34px] sm:text-sm lg:text-base xl:text-lg 2xl:text-xl xl:h-11">
                <div className="flex items-center h-full">
                  <div className="bg-[#ff0000] rounded-lg aspect-square h-7 xl:h-9 2xl:h-10" />
            ...
            {yellowPercent !== 0 && (
              <div className="flex items-center justify-between h-11 2xl:h-12 w-[96%] px-2 rounded-md item sm:h-[34px] sm:text-sm lg:text-base xl:text-lg 2xl:text-xl xl:h-11">
                <div className="flex items-center h-full">
                  <div className="bg-[#ffff00] rounded-lg aspect-square h-7 xl:h-9 2xl:h-10" />
            ...
            {greenPercent !== 0 && (
              <div className="flex items-center justify-between h-11 2xl:h-12 w-[96%] px-2 rounded-md item sm:h-[34px] sm:text-sm lg:text-base xl:text-lg 2xl:text-xl xl:h-11">
                <div className="flex items-center h-full">
                  <div className="bg-[#80ff00] rounded-lg aspect-square h-7 xl:h-9 2xl:h-10" />
            ...
            {pinkPercent !== 0 && (
              <div className="flex items-center justify-between h-11 2xl:h-12 w-[96%] px-2 rounded-md item sm:h-[34px] lg:text-base xl:text-lg 2xl:text-xl xl:h-11">
                <div className="flex items-center h-full">
                  <div className="bg-[#ff00ff] rounded-lg aspect-square h-7 xl:h-9 2xl:h-10" />
