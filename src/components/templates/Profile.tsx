...
export default function Profile({ profileData }: any) {
  ...
  useEffect(() => {
    if (languageSelected.code === "en") {
      ...
      if (profileData.kyc?.fname) {
        setProfileName(profileData.kyc.fname + " " + profileData.kyc.lname);
  ...
  return (
    <>
      ...
      <ProfileImages profileData={profileData} profileName={profileName} />
