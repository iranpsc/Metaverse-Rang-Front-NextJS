...
export default function ArticleCard({ item, params, theme, activeLoadingId, setActiveLoadingId }: ArticleCardProps) {
  ...
  return (
    <Link onClickCapture={() => setActiveLoadingId(item.id)} href={`/${params.lang}/news/categories/${item.categorySlug}/${item.slug}`} className={`${isLoading ? "rotating-border-card cursor-not-allowed" : ""} w-[100%] min-h-[240px] shadow-md hover:shadow-xl hover:dark:shadow-dark rounded-[10px] overflow-hidden bg-white dark:bg-[#1A1A18] flex flex-col justify-start gap-6 items-center`}>
      {isLoading && (
