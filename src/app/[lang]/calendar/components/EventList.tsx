...
const EventList = ({
  ...
  const showMore = async () => {
    ...
    // اگر هنوز آیتم‌های جدید از سرور نیامدن ولی کاربر بیشتر می‌خواهد:
    if (newVisibleCount > events.length) {
      try {
        const nextPage = currentPage + 1;
        const res = await fetch(
          `https://api.rgb.irpsc.com/api/calendar?page=${nextPage}&type=event`
        );
  ...
  const ThemedLoader = () => {
    ...
    useEffect(() => {
      ...
      // اگر کاربر تم رو تغییر داد، گوش کن (مثلاً از Tailwind toggle)
  ...
  //console.log("userLikedMap:", userLikedMap);
  //console.log("userDisLikedMap:", userDisLikedMap);

  const [cookies] = useCookies(["auth"]);
  const token = new URLSearchParams(cookies.auth).get("token");
  const sendLike = async (eventId: number) => {
    ...
    if (userLikedMap[eventId]) {
      console.log("کاربر قبلاً لایک کرده است.");
      return;
    }
    setLikesMap((prev) => ({
      ...prev,
  ...
  const disLike = async (eventId: number) => {
    ...
    if (userLikedMap[eventId] == false) {
      console.log("کاربر قبلاً دیس لایک کرده است.");
      return;
    }
    setDisLikesMap((prev) => ({
      ...prev,
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error("Response error details:", errorText); // اضافه کن
        throw new Error("خطا در ارسال لایک");
      }
    } catch (error) {
      console.error("خطا در ارسال لایک:", error);
    ...
    <>
      {visibleEvents.map((event) => {
        ...
        return (
          <div
            key={event.id}
            className="items flex flex-col justify-center  items-center w-full"
          >
                ...
                <div className="flex items-center  gap-1">
                  <svg
                    className={`cursor-pointer stroke-black dark:stroke-white ${
                      userLikedMap[event.id] ? "fill-black dark:fill-white" : "fill-none"
                    }`}
                ...
                <div className="flex items-center  gap-1 stroke-black dark:stroke-white fill-none">
                  <svg
                    className={`cursor-pointer rotate-180 stroke-black dark:stroke-white ${
                      userDisLikedMap[event.id] ? "fill-black dark:fill-white" : "fill-none"
                    }`}
              ...
              <div className="flex items-center justify-between h-8 w-full font-[Rokh] my-2 sm:w-[60%] xl:h-11">
                <div className="flex items-center h-full w-full">
                  <div
                    ...
                    className=" sm:h-7 xl:h-9 2xl:h-10 rounded-lg aspect-square h-full"
                  ></div>
