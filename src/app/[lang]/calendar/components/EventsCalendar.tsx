...
import type { EventItem } from "../page"; // مسیر را مطابق ساختار پروژه تنظیم کن
...
console.log(events)
  // تابع اضافه/حذف فیلتر
  function toggleFilter(color: string) {
    setSelectedFilters((prev) => {
      let updated: string[];
      if (prev.includes(color)) {
        // حذف اگر قبلا انتخاب شده بود
        updated = prev.filter((c) => c !== color);
      } 
     
      else {
        // اضافه اگر انتخاب نشده بود
        updated = [...prev, color];
      }
       if(color=="all"){
        updated=[];
      }
      console.log("فیلترهای انتخاب شده:", updated);
      return updated;
    });
  }
  ...
  return (
    <div className="centerItem w-[95%] lg:w-full  pt-6 text-black dark:text-white bg-white  dark:bg-[#080807] flex flex-col items-center rounded-[20px] gap-2 font-['Montserrat']  ">
      <div className="w-[97%]  flex flex-col items-start sm:flex-row-reverse lg:w-[95%] lg:gap-4 ">
        <Calendar mainData={mainData} params={params} />
        <div className="EventFilters w-full sm:w-[90%]  mt-4 sm:mt-0 sm:ml-4  ">
          ...
          >
            ...
            />
            <button className="searchButton font-normal text-[95%] px-5 font-['AzarMehr'] border-none bg-transparent text-[#FFBC00] cursor-pointer">
         ...
         ${getFilterClasses("yellow")} `}
          ...
          `}
            >
              <div className="flex items-center h-full ">
                <div className="bg-[#ff00ff] rounded-lg aspect-square h-8 xl:h-9  2xl:h-10 sm:h-7"></div>
                <span className="mx-2 whitespace-nowrap">
